...
async function FetchAllPosts() {
    try {
        const posts = await db.select('*').from('posts').where(is_deleted, false);
        return posts;
    } catch (error) {
...
async function FetchPostsByLocation(coordinates) {
    try {
        const posts = await db.select('*').select(
            db.raw(
                'ST_DISTANCE_SPHERE(POINT(?, ?), POINT(longitude, latitude))/1000 AS distance', [coordinates.longitude, coordinates.latitude]
            )
        ).select(
            db.raw(
                "CASE " +
